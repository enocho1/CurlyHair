...
Stretch::Stretch(int p0, int p1, double stiffness,double damping,  const dVector& X) {
	particle0 = p0;
	particle1 = p1;
...
double Stretch::globalK = 1;
bool Stretch::useGlobalK = false;

double Stretch::getStiffness() {
	if (useGlobalK) {
		return globalK;
	}
	else return springK;
...
void Stretch::computeForces(const dVector& x) {
	//enoch's code
...
void Stretch::addForcesToVector(const dVector& x, dVector& forceVector) {
	computeForces(x);
...
void Stretch::compute_dFdx(const dVector& x) {
...
void Stretch::addBlocksToMatrix(const dVector& x, MS_SparseSquare& forceJacobian) {
	compute_dFdx(x);
